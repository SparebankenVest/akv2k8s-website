...
Azure Workload Identity is supported by setting Key Vault authentication to `environment-azidentity`. See [Installing with Azure Workload Identity](../installation/with-azure-workload-identity) for setup instructions.
...
When used with Azure Workload Identity, the workload identity webhook injects the environment variables and token file required by `DefaultAzureCredential`.
